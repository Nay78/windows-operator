...
public static class OperatorOpenApi
{
    ...
    private static object BuildDocument()
    {
        ...
        var paths = new Dictionary<string, object>
        {
            ...
            ["/v1/powerpoint/inspect"] = Path(
                Post(
                    "inspectPowerPoint",
                    "Inspect PowerPoint slides and shapes for stable edit-plan targets.",
                    schema.Ref<PowerPointInspectRequest>(),
                    schema.Ref<PowerPointInspectResult>())),
            ["/v1/powerpoint/edit"] = Path(
                Post(
                    "editPowerPoint",
                    "Apply a typed PowerPoint edit plan through desktop PowerPoint COM.",
                    schema.Ref<PowerPointEditRequest>(),
                    schema.Ref<PowerPointEditResult>())),
            ["/v1/powerpoint/jobs/{jobId}"] = Path(
                Get(
                    "getPowerPointJob",
                    "Read a prior PowerPoint edit job manifest.",
                    schema.Ref<PowerPointEditResult>(),
